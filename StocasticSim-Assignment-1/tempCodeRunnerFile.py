...
def histOfEigenvalues(A, d, p_d):
    """Gives a histogram of the eigenvalues of A"""
    ...
    nr_bins = int((round(max(eigenvalues)) - round(min(eigenvalues)))/2)
...
def ErdosRenyi(p_d, d):
    """Generates an d x d Erdös-Rényi matrix A with bernoulli parameter p_d"""
    A = np.zeros((d,d))
    for row in range(d):
        for col in range(row+1, d):
            bit = rng.choice([0,1],1, p=[1-p_d, p_d])
            A[row][col] = bit
            A[col][row] = bit
    return A

def test_ErdosRenyi():
    assert (ErdosRenyi(0, 7) == np.zeros((7,7))).all(), 'If p_d = 0, ErdosRenyi() should output an all zero matrix'
    assert (ErdosRenyi(1, 7) == np.ones((1,1)) - np.identity(7)).all(), 'If p_d = 1, ErdosRenyi() should output an all one matrix'

test_ErdosRenyi()

def simulateSn(p_d, n, d):
    """Generate S_n = sum^n_i=1 X_i, where X_i is an d x d Erdös-Rényi matrix with bernoulli parameter p_d"""
    S = np.asmatrix(np.zeros((d,d)))
    for _ in range(n):
        S += np.asmatrix(ErdosRenyi(p_d, d))
    return S

def expectedSn(trials, p_d, n, d):
    """Returns the expected value of S_n, based on the number trials"""
    total = np.asmatrix(np.zeros((d,d)))
    for _ in range(trials):
        total += simulateSn(p_d, n, d)
...
def generateZ(sigma, d):
    ...
    for _ in range(d):
        ...
        Z += np.transpose(G)*G   #Not that sampling from the multivariate_normal gives a row vector, therefore we switch the transpose multiplication
    return Z
